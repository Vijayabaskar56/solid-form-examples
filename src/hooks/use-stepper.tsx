...
export function useFormStepper(schemas: ZodObject[]) {
	...
	const [currentStep, setCurrentStep] = createSignal(1); // Start from 1
	...
	const currentValidator = createMemo(() => schemas[currentStep() - 1]); // Convert to 0-based for array access
	const isFirstStep = createMemo(() => currentStep() === 1);

	const triggerFormGroup = async (form: AnyFormApi) => {
		const validator = currentValidator();
		const result = validator.safeParse(form.state.values);
		if (!result.success) {
